...
function EventRow({ event }) {
  ...
  const background = getBackgroundColor(event.severity);
  ...
  return (
    <>
      ...
      {logsVisible ? (
        <tr style={{ background, borderTop: 'none' }}>
          <td colSpan={5}>
            <div className="logRow">
              {event.logs.map((line, index) => (
                <div key={`${event.alertname}-log-${index}`}>{line}</div>
              ))}
            </div>
          </td>
        </tr>
    ...
    <div style={{ overflow: 'auto' }}>
      <table>
        ...
        <tbody>
          {events.map((event) => (
            <EventRow
              key={`${event.cluster}-${event.alertname}-${event.triggered}`}
              event={event}
            />
          ))}
        </tbody>
